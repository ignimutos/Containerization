...
github_tag() {
  ...
  # 优先使用 release tag；若仓库未发布 release，则回退到最新 git tag
  local tag=$(curl -s ${github_opts[@]} "https://api.github.com/repos/$repo/releases/latest" | jq -r '.tag_name // empty' | sed 's/^v//')
  if [[ -z $tag ]]; then
    tag=$(curl -s ${github_opts[@]} "https://api.github.com/repos/$repo/tags?per_page=1" | jq -r '.[0].name // empty' | sed 's/^v//')
  fi

  if [[ -z $tag ]]; then
    echo "Error: Unable to resolve latest tag for '$repo'" >&2
    return 1
  fi
